...
export default function ConciergeClientView({ threads: initialThreads }: { threads: ConciergeThread[] }) {
  const [threads, setThreads] = useState<ConciergeThread[]>(initialThreads)
  ...
  function handleStatusChange(customerId: string, newStatus: 'open' | 'closed') {
    setThreads((prev) =>
      prev.map((t) => t.customerId === customerId ? { ...t, status: newStatus } : t)
    )
  }
