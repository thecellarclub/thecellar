...
- Direct SQL against the `fqywjskvgkvgbtqzckqe` Supabase project: unlinked the 3 cellar rows (6 bottles, none `shipped_at`) from the shipment; restored `customers.case_started_at` to `2026-07-08 17:14:45` (the oldest of the 3 now-unlinked cellar rows' `added_at` — reconstructs what Scenario 1 would have originally set it to, so the day-90 reminder cron keeps tracking him correctly) and cleared `case_reminder_sent_at`. `free_shipping_at_6` was already `false` (auto-consumed at shipment creation) — nothing further needed there. Logged `inbox_activity` (`actor_id: null` — no `admin_users` row matches Julia's `julia@thebothy.club` email in production, see Gotchas; `action: 'shipment_paused'`, detail noting the admin request). Initially set the shipment's `status` to `paused` (same end state as the customer-initiated PAUSE flow) rather than deleting it outright, to preserve an audit trail — Julia then asked to remove it from the shipments page entirely, so it was deleted afterward (confirmed zero remaining FK references — `cellar.shipment_id` was already nulled by the unlink step above, and it's the only foreign key pointing at `shipments`).

**Deviations & decisions**
- Deleted the shipment row per Julia's explicit follow-up request, superseding the initial `paused`-not-deleted choice below. No `inbox_activity` row exists for the deletion itself (the row it would reference no longer exists) — the earlier `shipment_paused` entry stands as the audit trail for this repair.
...
**Verification**
- Confirmed post-repair via direct query: all 3 cellar rows `shipment_id: null`, `shipped_at: null`; customer `free_shipping_at_6: false`, `case_started_at: 2026-07-08 17:14:45`, `case_reminder_sent_at: null`; shipment row `6576bb7b` confirmed gone (0 rows) after the follow-up deletion.
...
**State changes**
- `.claude/settings.local.json`: removed the three tracked `Bash(export TWILIO_...)` permission entries (contained the live Account SID + Auth Token in plaintext) and the stray `Bash(export OUT_PATH)` entry. `git rm --cached`'d the file and added it to `.gitignore` — kept the local working copy. Confirmed `git log -S "TWILIO_AUTH_TOKEN" --all` finds nothing in history, so this was a working-tree-only exposure, never pushed.
- `app/api/cron/inbox-digest/route.ts`: fail-open auth bug fixed — was `if (secret && header !== ...)`, so an unset `CRON_SECRET` ran the route unauthenticated. Now matches the other three crons' strict `header !== \`Bearer ${process.env.CRON_SECRET}\`` comparison.
- `app/api/signup/send-code/route.ts`, `app/api/portal/send-otp/route.ts`: OTP generation switched from `Math.random()` to `crypto.randomInt(100000, 1000000)`.
- `app/api/portal/send-otp/route.ts`: added the same IP rate limit as signup (`ip:` key, 10/hour) ahead of any DB work. `app/api/portal/verify-otp/route.ts`: added a 20/hour IP limit (the per-code attempt cap is still the primary guard).
- `lib/auth.ts`: `authorize()` now rate-limits by `admin-login:{email}` (10 attempts / 15 min) before touching the DB; `session.maxAge` dropped from NextAuth's default 30 days to 7 days.
- Four admin routes standardised on `requireAdminSession()` (was bare `getServerSession(authOptions)`): `customers/[id]/refund`, `customers/[id]/cancel-order`, `customers/[id]/add-bottles`, `admin/broadcast`. No behaviour change — same 401 on missing session, now also gets the stale-UUID-session re-resolution the other admin routes already had.
- `app/api/webhooks/twilio/inbound/route.ts`: the signed-URL reconstruction now uses the existing `APP_URL` constant (`NEXT_PUBLIC_APP_URL`) directly in production instead of trusting `host`/`x-forwarded-proto` headers; still falls back to header-based reconstruction outside production for ngrok.
- `next.config.ts`: was an empty config — added `poweredByHeader: false` and a `headers()` returning HSTS, `X-Content-Type-Options`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy` on `/:path*`. No CSP (explicit non-goal — Stripe Elements needs one built deliberately).
- **Migration `050_security_advisor_fixes.sql`** (applied via Supabase MCP): recreated `customer_cellar_totals` with `security_invoker = true` (was `SECURITY DEFINER`, an ERROR-level advisor finding) — preserved the 039 view definition (`shipment_id IS NULL`) verified live rather than assuming schema.sql. Pinned `search_path = public, pg_temp` on `increment_offers_received(uuid)` and `apply_credit(uuid, integer, text, text, uuid, uuid)` (WARN-level mutable-search-path findings) — looked up the real `apply_credit` signature via `pg_proc` first since it takes several args. **Numbered 050, not the spec's suggested 049** — 049 was already taken same-day by the unrelated `049_milestone_seven.sql` (tiers-v3.1 work). Re-ran `get_advisors(type: 'security')` after applying: both findings cleared, only the expected `rls_enabled_no_policy` INFO lints remain.
- `app/api/admin/customers/[id]/erase/route.ts`: now also nulls `last_name`, `dob`, `default_address`, `backup_payment_method_id`, `billing_token`/`billing_token_expires_at`, and the five UTM fields on the customer row. Added cleanup of side tables keyed by customer_id or phone (fetched before the row is overwritten): deletes `concierge_messages`, `sms_parse_log` (by `customer_id` OR `inbound_phone`), `verification_codes` (by phone), `inbox_notes`; nulls `shipping_address` on the customer's `shipments` rows (kept, per accounting integrity). Response now returns per-table removal counts (no PII) instead of just `{ ok, erasedAt }`.
- `app/api/cron/case-nudges/route.ts`: added a purge step at the end (runs daily already) — deletes `verification_codes` older than 24h and `sms_parse_log` rows older than 90 days, returns `codesPurged`/`parseLogPurged` counts in the JSON response.
- `app/api/ship/[token]/route.ts`: trimmed the `customers(...)` select from `first_name, email, phone` to just `first_name` — cross-checked against `app/ship/page.tsx` (the actual page rendering this data), which already only ever selected/rendered `first_name`. Note: this GET route isn't currently called by any frontend (`ShipForm.tsx` only posts to `/api/ship/confirm`; the `/ship` page does its own direct Supabase query) — looks like a now-orphaned endpoint, but still a live unauthenticated one, so trimmed it regardless.
- `app/api/authenticate/confirm/route.ts`: now takes `{ token }` instead of `{ orderId }`, looks the order up by `auth_token`, and re-checks `isAuthTokenExpired(order.created_at)` (previously only checked at initial page load, not at confirm time). `app/authenticate/page.tsx` passes `token` through to `AuthenticateForm` instead of `order.id`; `AuthenticateForm.tsx`'s `Props`/fetch body updated to match.
- `lib/auth.ts`: removed the `ADMIN_EMAIL`/`ADMIN_PASSWORD_HASH` env-var fallback entirely — confirmed via direct query that all three `admin_users` rows have real bcrypt hashes (`password_hash like '$placeholder%'` → false for all three, hash length 60). `authorize()` is now a straight DB lookup + `bcrypt.compare`, no try/catch-and-fall-through. Julia still needs to remove the two env vars from Vercel (not touched by Claude Code, per the spec).
- `lib/session.ts`: `sessionOptions.password` now reads `SIGNUP_SESSION_SECRET` with a fallback to `NEXTAUTH_SECRET` — no code changes needed once Julia adds the new var in Vercel; falls back safely until then.

**Deviations & decisions**
- Migration numbered **050**, not the spec's suggested 049 — see above (049 was claimed same-day by unrelated work).
- Did not implement the two `[JULIA]`-marked items (repo-visibility check, Twilio token rotation) — per the spec, those are Julia's operational actions. **Julia confirmed in-session that the GitHub repo privacy check (item 1) was done**, which the spec gated all other work on ("do not proceed past item 2 until confirmed") — proceeded on that basis. **Item 3 (rotating the Twilio Auth Token) was still pending as of this implementation** — the old token value was already scrubbed from the tracked permissions file (item 2), but it hasn't been rotated in the Twilio console/Vercel/`.env.local` yet.
- Did not add a `.env.example` entry for `SIGNUP_SESSION_SECRET` — no such file exists in the repo to add it to.

**Gotchas & future context**
- `app/api/ship/[token]/route.ts` (the GET-by-token route, distinct from `/api/ship/confirm`) appears to have no live caller in the frontend anymore — worth confirming with Julia whether it's dead code to remove, or whether something external still hits it.
- The `admin_users` email addresses in the live DB (`daniel@crushwines.co`, `members@thecellar.club`, `hello@crushwines.co`) don't match the emails CLAUDE.md documents for the team (`daniel@thecellar.club`, `julia@thebothy.club`, `craig@thecellar.club`) — flagging in case that's meant to be cleaned up separately; not touched here since it's out of scope for this spec and admin login is by DB row, not by CLAUDE.md prose.
- Did not exercise the OTP send flows, the erasure route, or a live Twilio inbound event end-to-end against production — doing so would send a real SMS / mutate a real customer record, which needs explicit sign-off outside this pass. Verified via `tsc`, `eslint`, `next build`, and a local dev server hit against all four cron routes (confirmed 401 with no/wrong bearer) and the new response headers (confirmed present, `X-Powered-By` absent) instead.

**Verification**
- `npx tsc --noEmit`: clean except the same pre-existing `lib/*.test.ts` test-runner type errors noted in prior entries (unrelated, untouched).
- `npx eslint` on all seventeen touched files: 0 new errors/warnings. One pre-existing `prefer-const` error in `refund/route.ts` (a `let order_id` a few lines below the auth-check swap, never touched) and one pre-existing unused-var warning in the Twilio webhook (`cardSavedOrderRecap`, from the tiers-v3.1 work) — both pre-existing, not introduced here.
- `npx next build`: clean, all 72 routes compile.
- Local dev server: all four `/api/cron/*` routes return 401 with no `Authorization` header and with a wrong bearer token. Security headers (`Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`) confirmed present on a page response; `X-Powered-By` confirmed absent.
- Migration 050 applied; `get_advisors(type: 'security')` re-run and confirmed both the `security_definer_view` ERROR and both `function_search_path_mutable` WARNs are gone — only the expected `rls_enabled_no_policy` INFOs remain.
- Not exercised: OTP send/verify end-to-end, admin login rate-limiting under real load, the erasure route against a real record, a genuine Twilio inbound signature check in production — see Gotchas above.

---

### 2026-07-22 — Tiers v3.1: ladder reshuffle + club page rewrite (`claude-code-prompt-tiers-v3-1.md`, `claude-code-prompt-club-page-v3.md`)

**State changes**
- `lib/tiers.ts`: `rebatePctForTier` elvet 0.10→0.07 (bailey 0.05, palatine 0.10 unchanged). `deliveryFeePence` palatine 500→**0**. `deliveryThreshold` no longer special-cases `tier === 'palatine'` — only the one-shot `free_shipping_at_6` flag returns 6, everyone (including Palatine) is 12 otherwise; Palatine's perk is now zero-fee delivery at any bottle count, not an early auto-complete. Elvet/Palatine congrats SMS copy updated (Elvet: "climbs to 7%"; Palatine: leads with 10%/2hr-early/free-shipping-anytime, drops the Coravin mention, teases "one more case" instead since Coravin no longer coincides with this upgrade).
- `lib/milestones.ts`: `MILESTONES`/`AUTO_REWARD`/`MILESTONE_OPTIONS`/`REWARD_LABELS` now exported as the single source of truth (previously `MILESTONE_OPTIONS`/`REWARD_LABELS` were duplicated in three places — the admin milestones page, the PATCH route, and implicitly in the portal). Milestone set `{1,3,5,6}` → `{1,3,5,7}`; Coravin's `AUTO_REWARD` entry moved from 6 to 7 with new SMS copy ("Seven cases. Your Coravin Timeless is on its way...") and its own `notifyAdmin` call. **Milestone 3/5's reward assignment (free bottle at 3, Riedel at 5) was already correct in the live code** — the spec's "swap" description matched the original `claude-code-prompt-tiers-v3.md` draft table (Riedel at 3, bottle at 5), but the actual implementation already shipped with 3=bottle/5=Riedel back on 2026-07-13, so no reassignment was needed here, only centralisation. Removed the `skipSmsForMilestone` opt entirely — milestone 6 used to coincide with the Palatine upgrade (one combined SMS); milestone 7 (Coravin) is now a case ahead of Palatine (case 6), so they're always separate events.
- `lib/post-charge.ts`: `awardMilestones(customerId, sb)` called with no opts (the combined-SMS suppression logic it used to pass is gone).
- **Migration `049_milestone_seven.sql`** (applied via Supabase MCP `apply_migration`): looked up the real constraint name first (`milestone_awards_milestone_check`, confirmed via `pg_constraint` — matched the auto-generated guess but verified rather than assumed per the spec's instruction). Guarded with a `do $$ ... raise exception` block if any `milestone = 6` rows exist before dropping/recreating the constraint as `milestone in (1, 3, 5, 7)` — confirmed 0 such rows via direct query beforehand. Migration numbered **049**, not the spec's suggested 047 — 047 and 048 were already taken by unrelated migrations shipped 2026-07-21 (`047_remove_case_deadline.sql`, `048_enable_rls_credit_and_milestones.sql`); the spec itself flagged "verify latest number first."
- `app/api/admin/milestones/[id]/route.ts`, `app/admin/(protected)/milestones/page.tsx`, `app/portal/dashboard/DashboardClient.tsx`: all now import `MILESTONE_OPTIONS`/`REWARD_LABELS` from `lib/milestones.ts` instead of maintaining their own copies. `MilestoneRowActions.tsx`'s comment updated (milestone 6→7).
- `scripts/backfill-milestones.ts`: milestone set and `AUTO_REWARD` updated to match `{1,3,5,7}`. Nobody is currently at milestone 6 or 7 (verified live), so this is a no-op if rerun — just keeps the script correct.
- `app/api/webhooks/twilio/inbound/route.ts`: **`handleShipConfirm`** restructured — the cellar-rows fetch that used to happen after the "no card on file" guard now happens before it, and a new branch fires when `deliveryFeePence(tier) === 0`: creates the shipment directly (`shipping_fee_pence: 0`, no Stripe fields) and skips both the card-on-file guard and the Stripe `paymentIntents.create` call entirely (Stripe rejects zero-amount PaymentIntents; mirrors the existing full-credit-redemption skip-PI pattern in the YES handler). A Palatine customer with no card on file can now get their free shipment without being asked to add one. **`handleShip`**'s under-threshold fee-quote message now says "Shipping's free for you, any time" instead of "Shipping now costs £0" when the fee is zero. **`handleStatus`**'s cellar-summary line says "free shipping any time" for Palatine instead of "free shipping at {threshold}" (which would otherwise misleadingly read 12).
- `app/portal/dashboard/DashboardClient.tsx`: line 320's hardcoded `tier === 'palatine' ? 6 : 12` threshold removed (always 12 now — this component doesn't track the `free_shipping_at_6` flag at all, a pre-existing gap left as-is). The cellar-summary line shows "free shipping any time" for Palatine instead of "free ship at 12".
- **Copy sweep**: grepped `lib/` and `app/` (excluding the club page, which got a full rewrite) for stale "10%"-Elvet, "free shipping at 6"-Palatine, and Coravin-at-6 references — none found outside the two intentionally-updated files. Also fixed `app/page.tsx`'s "How much does it cost to join?" FAQ answer, which still said "discounts" and "rolling twelve-month spend" — pre-v3 copy that predates even the original tiers-v3 ship (only the *delivery-fee* FAQ line was updated back then, per that entry's own note) and directly contradicts both this spec's and the original tiers-v3 spec's explicit "no discount, no rolling spend" rules. Not explicitly named in either spec's file list, but caught by the tiers-v3-1 copy-sweep instruction's spirit; flagging as a deviation since it's editing a line neither spec named directly.
- `app/club/page.tsx`: full rewrite per `claude-code-prompt-club-page-v3.md` — header, 3-line "how it works" intro, 7-rung ladder (cases 1–7, Coravin now the case-7 reward), three tier detail blocks (Bailey/Elvet/Palatine, in that order, each showing its case count + 4 perk rows), credit/gifts/membership-year explainer sections, footnote + `/join` CTA. `metadata.title` unchanged ("How the Club works - The Cellar Club" — already matched the spec's suggestion from the original 2026-07-13 ship).

**Deviations & decisions**
- **The spec's "how it works" intro (§2) and "tier detail" blocks (§4) don't exist in the live page and referenced a `PerkEntry` component that doesn't exist anywhere in the codebase.** Grepped for `PerkEntry` and for the dotted-leader row pattern across the whole `app/` tree — the only place either exists is `app/club/page.tsx` itself, in its own `LadderRow`. Cross-checked against git history: the original 2026-07-13 rewrite *did* ship both sections, but they were trimmed by follow-up commits (`a2420de` "drop how-it-works blurb", `4ced5bd` "trim redundant tier detail") sometime after go-live, none of which have a matching `IMPLEMENTATION-LOG` entry — likely small direct polish passes, not separately logged. Since `claude-code-prompt-club-page-v3.md` is dated "Updated 2026-07-22" and explicitly re-specifies both sections with full copy as part of "a full content rewrite," implemented it as written (per CLAUDE.md: don't second-guess a spec) rather than assuming the trim was meant to stick. Built a new minimal `PerkEntry` (dotted-leader label/value row) and `TierBlock` component matching the page's existing visual language, since there was nothing to actually reuse.
- Palatine congrats SMS (`lib/tiers.ts`) says "no more delivery fees, ever" and teases the Coravin one case away — not verbatim from the spec (which only gave bullet points: "lead with 10% credit back, texts 2 hrs early, and free shipping anytime... Can tease 'one case from your Coravin' if it reads well"), but keeps the meaning. Julia will polish per the file's existing "draft copy" comment.
- Left `case-nudges` cron untouched — it has no delivery-fee logic left to update (the entire auto-ship-charge block was already deleted in the 2026-07-21 "remove the 90-day case deadline" work), so the spec's mention of "case-nudges cron's auto-ship charge" is now moot.
- Did not touch `.claude/settings.local.json`'s tracked, plaintext Twilio credentials (spotted while checking git status before starting this work, unrelated to either spec) — flagged to Julia in chat, not actioned without her direction.

**Gotchas & future context**
- `lib/milestones.ts` is now the only place milestone reward options/labels/set are defined — anything that used to hardcode `VALID_CHOICES`/`MILESTONE_OPTIONS` locally should import from there instead of re-declaring, or it'll drift again next time the reward menu changes (this is the second time it's changed).
- `deliveryThreshold(tier, freeShippingAt6)` still takes `tier` as a parameter even though it's now unused inside the function (only `freeShippingAt6` matters) — kept for call-site stability rather than touching every call site's signature. Don't be surprised it's a no-op parameter.
- Zero Palatine customers exist in production as of this session (confirmed via the tier distribution from the 2026-07-13 recompute — 2 bailey, rest none), so none of the £0-delivery/skip-Stripe-PI code paths, the milestone-7 award path, or the new club-page copy have been exercised against a real Palatine customer yet. First real Palatine upgrade and first genuine 7-case customer will be the first live exercise of this entire spec.

**Verification**
- `npx tsc --noEmit`: clean except the same pre-existing test-runner type errors in `lib/*.test.ts` noted in every prior entry (unrelated, untouched).
- `npx eslint` on all eleven touched files: 0 errors, 3 pre-existing unrelated warnings (`wineName` in post-charge.ts, `cardSavedOrderRecap` in the webhook, a `useEffect` dependency warning in DashboardClient — none introduced by this change).
- `npx next build`: clean, `/club` prerenders as static (`○`).
- Migration 049 applied and verified via direct `pg_constraint` query before and after (`milestone in (1, 3, 5, 6)` → `milestone in (1, 3, 5, 7)`); confirmed 0 pre-existing `milestone = 6` rows before applying.
- Grepped for stale "10%"-Elvet, "free shipping at 6"-Palatine, and Coravin-at-6 copy across `lib/` and `app/` (excluding the rewritten club page) — none found. Grepped the rewritten club page for "spend"/"£500"/"£1,000"/"discount"/"rolling twelve-month" — none found.
- Not exercised against a live Twilio/Stripe flow or a real Palatine customer (none exist yet — see Gotchas) — verified by full code review and tracing every changed path against the live schema and current production data (milestone counts, constraint state, tier distribution) via direct Supabase queries.
